...
mod tests {
    ...
    fn try_read_from_empty_buffer_returns_read_failure_error() {
        let (_producer, mut consumer) = RingbufRuntime::create_named_channel::<f32>(4);
    ...
    fn try_read_after_write_returns_the_written_sample() {
        let (mut producer, mut consumer) = RingbufRuntime::create_named_channel::<f32>(4);
    ...
    fn read_into_fills_at_most_the_available_samples() {
        ...
        let (mut producer, mut consumer) = RingbufRuntime::create_named_channel::<f32>(CAPACITY);
    ...
    fn drain_on_empty_buffer_returns_empty_vec() {
        const CAPACITY: usize = 4;
        let (_producer, mut consumer) = RingbufRuntime::create_named_channel::<f32>(CAPACITY);
