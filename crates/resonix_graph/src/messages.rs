...
pub enum NodeMessageError {
    #[error("No corresponding node found in the graph for node with uuid {uid:?} at node index {node_index:?}")]
    NodeNotFound { uid: u32, node_index: NodeIndex },
    #[error("Node message was sent for the wrong node type")]
    WrongNodeType { uid: u32, node_index: NodeIndex },
}
