...
use crate::{
    AddConnectionError, BoxedNode, Connection, DACNode, Node, NodeType, NonSharedGraph, SharedGraph,
};
...
impl Processor<SharedGraph> {
    ...
    pub(crate) fn run(&mut self) {
        ...
        for node_index in self.visit_order.as_ref().unwrap() {
            ...
            let mut node = self.graph[*node_index];
            ...
            let mut outgoing_connections = {
                outgoing_edge_indexes
                    .iter()
                    .map(|i| self.graph.edge_weight_mut(*i).unwrap().borrow_mut())
            };
    ...
    pub fn add_node<N: Node + 'static>(&mut self, node: N) -> Result<NodeIndex, AddNodeError> {
        ...
        let node_index = self.graph.add_node(Rc::new(RefCell::new(Box::new(node))));
    ...
    fn connect_with_indexes(
        ...
        let edge_index = self.graph.add_edge(
            parent_node_index,
            child_node_index,
            Rc::new(RefCell::new(Connection::from_indexes(from_index, to_index))),
        );
...
impl<G: Default> Processor<G> {
    pub fn new() -> Self {
        Default::default()
    }
}

impl<G> Deref for Processor<G> {
    type Target = G;
...
impl<G> DerefMut for Processor<G> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.graph
    }
}

impl From<Processor<NonSharedGraph>> for Processor<SharedGraph> {
    fn from(value: Processor<NonSharedGraph>) -> Self {
        Processor {
            graph: value.graph.into(),
            node_uuids: value.node_uuids,
            visit_order: value.visit_order,
            input_node_indexes: value.input_node_indexes,
            dac_node_indexes: value.dac_node_indexes,
        }
    }
}
...
#[cfg(test)]
mod test_processor {
    use crate::{ConstantNode, DACNode, PassThroughNode, Processor, SharedGraph};

    #[test]
    fn running_processor_should_fill_connections_with_data() {
        let mut processor = Processor::<SharedGraph>::default();
